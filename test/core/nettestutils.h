#ifndef TEST_CORE_NETTESTUTILS_H
#define TEST_CORE_NETTESTUTILS_H
...
namespace NetTestUtils
{
    ...
    inline int createBlockingTcpSocket()
    {
    ...
    {
        return createBlockingTcpSocket();
    }
    ...
    inline void writeEventFd(int fd, uint64_t value = 1)
    {
    ...
    inline void connectToPort(int fd, uint16_t port)
    {
    ...
    inline int connectClientToPort(uint16_t port)
    {
    ...
    inline int getSocketOption(int fd, int level, int option)
    {
        int value = -1;
    ...
    inline sockaddr_in makeSockAddr(const char *ip, uint16_t port)
    {
        sockaddr_in addr{};
    ...
    inline ConnectedTcpSockets createConnectedTcpSockets()
    {
    ...
    inline short waitForFdEvent(int fd, short events, int timeoutMs)
    {
        struct pollfd pfd
        {
            fd, events, 0
    ...
    inline int pollReadableOrHangup(int fd, int timeoutMs)
    {
        return waitForFdEvent(fd, POLLIN | POLLHUP, timeoutMs);
    }

    inline std::string readExactly(int fd, size_t expectedBytes, int timeoutMs = 2000)
    {
        std::string data;
    ...
    inline std::string readUntilEof(int fd, int timeoutMs = 2000)
    {
        std::string data;
        char buffer[4096];
        const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(timeoutMs);
    ...
    inline std::map<int, ListeningSocketInfo> snapshotListeningTcpSockets()
    {
    ...
    inline std::optional<ListeningSocketInfo> findNewListeningSocket(const std::map<int, ListeningSocketInfo> &before)
    {
    ...
    inline std::optional<ListeningSocketInfo> findListeningSocketInfo(std::string_view ip = "127.0.0.1")
    {
    ...
    inline int countListeningTcpSockets(std::string_view ip = {})
    {
        int count = 0;
        const auto sockets = snapshotListeningTcpSockets();
        for (const auto &[fd, info] : sockets)
    ...
    inline TcpConnectionPtr createEstablishedConnection(
        EventLoop *loop,
        ConnectedTcpSockets &sockets,
        const std::function<void(const TcpConnectionPtr &)> &configure)
    {
    ...
    inline void destroyConnection(EventLoop *loop, const TcpConnectionPtr &conn)
    {
    ...
    inline ScopedFd createTempFileWithContent(const std::string &content)
    {
    ...
    inline std::shared_ptr<TcpServer> createAndStartServer(
        EventLoop *loop,
        int threadNum,
        const std::function<void(TcpServer &)> &configure,
        uint16_t *port)
    {
    ...
    inline void destroyServerOnLoop(EventLoop *loop, std::shared_ptr<TcpServer> &server)
    {
